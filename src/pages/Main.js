    ...
    <SafeAreaView style={styles.container}>
        ...
        { users.length === 0
          ...
          : (users.map((user, index) => (
            <View key={user._id} style={[styles.card, { zIndex: users.length - index }]}>
              <Image style={styles.cardAvatar} source={{ uri: user.avatar }} />
              <View style={styles.footer}>
                <Text style={styles.footerName}>{user.name}</Text>
                <Text style={styles.footerBio} numberOfLines={3}>{user.bio}</Text>
              </View>
            </View>
          )))
        }
      ...
      { matchDev && (
        <View style={[styles.matchContainer, { zIndex: users.length + 1 }]}>
          ...
          <TouchableOpacity onPress={_ => setMatchDev(null)}>
